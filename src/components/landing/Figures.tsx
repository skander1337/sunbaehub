...
export function BookingFigure({ t, days, price, durationMin = 60 }: Common & { days: FigureDay[]; price: number; durationMin?: number }) {
  ...
  return (
    <div className="card flex h-full flex-col p-4" aria-hidden="true">
      <div className="text-[12px] font-semibold text-ink-3">{t("profile.pickDay")}</div>
      <div className="mt-2 flex gap-1.5">
        {days.slice(0, 5).map((d, i) => (
          <span
            key={`${d.dayName}-${d.dateLabel}`}
            className={`flex w-[52px] flex-col items-center rounded-[12px] px-1 py-1.5 text-center ${
              i === active ? "bg-ink text-white" : d.times.length ? "bg-mist text-ink" : "text-ink-3/50"
            }`}
          >
            <span className="text-[11px] font-medium">{d.isToday ? t("profile.today") : d.dayName}</span>
            <span className="tnum text-[14px] font-bold">{d.dateLabel}</span>
          </span>
        ))}
      </div>
      <div className="mt-4 text-[12px] font-semibold text-ink-3">{t("profile.pickTime")}</div>
      ...
      <div className="mt-auto border-t border-line pt-3">
        ...
        <div className="mt-1 text-[12px] text-ink-3">{t("landing.fig.holdNote")}</div>
  ...
  return (
    <div className="card flex h-full flex-col overflow-hidden" aria-hidden="true">
      <div className="flex items-center justify-between gap-3 border-b border-line px-4 py-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[11.5px] text-ink-3">{t("session.title")}</div>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col gap-3 px-4 py-4">
        ...
        <div className="flex justify-end">
          <div className="flex max-w-[88%] flex-col items-end">
            ...
            <div className="mt-1 text-right text-[11.5px] text-ink-3">
      ...
      <div className="flex items-center gap-2 border-t border-line px-3 py-2.5">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center text-ink-3">
          <IconPaperclip size={16} />
        </span>
        <span className="flex h-9 min-w-0 flex-1 items-center truncate rounded-[12px] border border-line px-3 text-[13px] text-ink-3">{t("session.placeholder")}</span>
...
export function ReviewFigure({
  ...
  return (
    <div className="card flex h-full flex-col p-4" aria-hidden="true">
      <div className="text-[12px] font-semibold text-ink-3">{t("review.score")}</div>
      <div className="mt-1 flex items-baseline justify-between gap-3">
        <span className="tnum text-[40px] leading-none font-extrabold tracking-[-0.03em]">
          {score}
          <span className="text-[15px] font-semibold text-ink-3">/100</span>
        </span>
        <span className={`text-[13px] font-semibold ${score >= 75 ? "text-success" : score >= 60 ? "text-ink-2" : "text-danger"}`}>{hint}</span>
      </div>
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-2.5">
      <div className="min-w-0">
        ...
        <div className="text-[12px] text-ink-3">{party}</div>
      </div>
  ...
  return (
    <div className="card p-5 sm:p-6" aria-hidden="true">
      <div className="flex items-baseline justify-between gap-3">
        <div className="text-[13px] font-semibold text-ink-3">{t("landing.fig.ledgerTitle")}</div>
      ...
      <div className="mt-4 text-[12px] font-semibold text-success">{t("landing.creditsRow2")}</div>
      <div className="mt-1 divide-y divide-line border-y border-line">
        <LedgerRow label={t("tx.booking_release")} party={sunbae} amount={normal.specialistPayout} tone="text-success" />
        <LedgerRow label={t("tx.platform_fee")} party={platform} amount={normal.platformFee} tone="text-ink-2" />
      </div>
      <div className="mt-4 text-[12px] font-semibold text-danger">{t("landing.creditsRow3")}</div>
      <div className="mt-1 divide-y divide-line border-y border-line">
        <LedgerRow label={t("tx.booking_refund")} party={seeker} amount={split.seekerRefund} tone="text-brand" />
        <LedgerRow label={t("tx.booking_release")} party={sunbae} amount={split.specialistPayout} tone="text-success" />
